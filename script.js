document.addEventListener('DOMContentLoaded', function () {
  ...
  function getRequiredMessage(fieldName) {
    ...
    return capitalized + ' is required.';
  }
  ...
  function clearFormErrors(form) {
    ...
    if (successMsg) {
      successMsg.parentNode && successMsg.parentNode.removeChild(successMsg);
    }
  }
  ...
  const OBSERVE_TARGETS = '.fade-in, .animate-on-scroll, [data-animate]';

  function initScrollAnimations() {
    const elements = document.querySelectorAll(OBSERVE_TARGETS);
  ...
  const mutationObserver = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      mutation.addedNodes.forEach(function (node) {
        ...
        if (node.matches && node.matches(OBSERVE_TARGETS)) {
          initScrollAnimations();
        } else if (node.querySelector && node.querySelector(OBSERVE_TARGETS)) {
          initScrollAnimations();
        }
      });
    });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "כהן לפידיון הבן משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '685850f7-8720-4d1f-9f1d-d1fbfd62477a',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '685850f7-8720-4d1f-9f1d-d1fbfd62477a';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '685850f7-8720-4d1f-9f1d-d1fbfd62477a';
